...
2. Generate the work-order-specific sign-off templates with
   `pnpm run check:manual-review`.
3. Run each validator command listed in the work order.
4. Exercise the named workflow in the packaged app or current local app.
5. Capture screenshots, exported artifacts, or native-viewer evidence paths.
6. Record every checklist item as pass or as a non-release-blocking exception
   with a specific rationale.
7. Save the completed sign-off as
   `.tmp/manual-review/<work-order-id>/signoff.json`.
8. Put screenshots or related artifacts under
   `.tmp/manual-review/<work-order-id>/artifacts/`.
9. Ingest the evidence with
   `pnpm run ingest:evidence -- --source .tmp/manual-review/<work-order-id>`.
10. Re-run `pnpm run check:manual-review`,
   `pnpm run check:release-readiness`, and
...
- `pnpm run check:manual-review` accepts the sign-off, and
  `pnpm run ingest:evidence` copies it to
  `.tmp/manual-review/external/<work-order-id>/signoff.json`.
