...
const runbooks = [
  {
    ...
    commands: [
      ...
      "pnpm run build",
      "./node_modules/.bin/tauri build --bundles nsis",
      "Optional diagnostic: ./node_modules/.bin/tauri build --bundles msi",
  ...
  },
  {
    ...
    commands: [
      ...
      "Run pnpm run collect:manual-review -- --work-order-id <work-order-id> --template templates/spec-manual-review/<work-order-id>.template.json --reviewer-name \"Reviewer Name\" --platform-version \"OS Version\" --platform-device \"Device\" --artifact-file /path/to/screenshot-or-export-proof.png --validator-output-file /path/to/validator-output.txt --notes \"Reviewed the assigned workflow and found no release blockers.\".",
      "Return the generated signoff directory or run pnpm run ingest:evidence -- --source .tmp/manual-review/external/<work-order-id>.",
      "pnpm run check:manual-review",
      "pnpm run check:release-readiness",
    ],
