...
| 9.5 AI provenance | `ai-source`, AI-assisted sections, export appendix | Partial | `provenance.rs`; review/provenance tests; UI toggles; Playwright AI paste governance workflow covers `ai-source` insertion and draft review markers | Export readiness proof for unreviewed content and appendix workflow coverage. |
...
| 9.12 Bibliography | BibTeX/CSL JSON, citation syntax, rendered bibliography | Partial | `bibliography.rs`; citation tests; Playwright harness covers command-palette insertion of `[BIBLIOGRAPHY]` | CSL style fidelity, UI manager workflow, duplicate-key UI, and native workflow execution. |
