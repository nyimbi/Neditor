...
| 18 Export system | Targets, options, manifest | Partial | Export modules/tests and UI; `export_option_matrix_is_preserved_across_targets_and_bundle_evidence` proves one explicit export option matrix across every target artifact family plus Markdown bundle manifest evidence | Artifact-level completion audit, more option combinations, and cross-platform packaging proof. |
