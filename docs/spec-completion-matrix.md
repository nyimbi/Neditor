...
| 2 Source Prompt Extension | Side-by-side Markdown editor and live preview | Partial | `src/App.vue` editor/preview panes; CodeMirror setup; Playwright harness covers source edit to live preview update; archived browser workflow run `26153224371` covers synchronized editor/preview scrolling and preview heading click-to-source; `tests/frontend-unit.test.ts` proves preview debounce timing stays within the small-document target budget and coalesces rapid edits | Desktop execution and large-document native/browser interaction proof. |
...
- Accessibility has an initial static guard, but full keyboard/manual audit
  ...
  cancellation coverage, and preview debounce timing/coalescing coverage, but
  memory growth and native/browser performance workflow proof remain
  under-proven.
