...
| 9.12 Bibliography | BibTeX/CSL JSON, citation syntax, rendered bibliography | Partial | `bibliography.rs`; citation tests; Playwright harness covers command-palette insertion of `[BIBLIOGRAPHY]`, resolved reference display, missing-key UI, and duplicate-key UI; `prepare_for_export_reports_missing_citation_sources_with_precise_ranges` proves no-bibliography citation readiness ranges and manifest parity | CSL style fidelity, richer manager workflows, duplicate-key source locations, and native workflow execution. |
