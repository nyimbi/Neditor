...
| 6.5 File Operations | Recent docs/folders, workspace restore | Partial | Persisted workspace store; Playwright mocked workflows cover workspace listing after open and recently closed reopening in CI run `26137556147`; browser CI run `26147556750` covers restart-style restore for open tabs, active tab, pinned state, mode/sidebar, workspace root, and recent files; browser CI run `26148828614` covers scroll restore and missing-restored-file warning coverage | Moved/renamed/deleted recently-closed behavior and fuller recent folder behavior. |
