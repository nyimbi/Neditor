...
| 6.3 Editor | Line numbers, word wrap, spellcheck, find/replace, word count | Partial | Settings and CodeMirror extensions; status bar stats; Playwright harness covers word/character/reading-time status metrics; archived browser workflow run `26154535588` covers line-number and word-wrap persistence plus CodeMirror find/replace | Spellcheck proof and local browser/native execution proof for status metrics. |
