...
- Accessibility has an initial static guard, but full keyboard/manual audit
  evidence is still missing. Performance now has compiler and repeated export
  loop stress coverage, but repeated editing sessions, memory growth, UI
  debounce timing, transform cache stress, progress reporting, and cancellation
  behavior remain under-proven.
