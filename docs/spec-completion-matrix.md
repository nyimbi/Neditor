...
| 8 Front Matter And Variables | YAML metadata, layout/export controls, variable resolution, filters | Partial | `front_matter.rs`; `variables.rs`; export options; `compiler_formats_document_variables_and_reports_bad_filters` proves `default`, `trim`, `upper`, `title`, `currency`, `percent`, and `round` filters plus unsupported/nonnumeric filter diagnostics; `formatted_document_variables_survive_cross_target_exports` proves formatted variable output survives HTML, PDF, DOCX, PPTX, and Markdown bundle text | Broader UI docs/workflow proof and malformed front matter diagnostics. |
