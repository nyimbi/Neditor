...
| 9.1 Git-free snapshots | Local snapshots for non-Git users | Partial | `snapshot.rs`; snapshot UI/tests; `snapshot_restore_is_scoped_to_active_document_store` and `snapshot_restore_rejects_out_of_scope_and_mismatched_sources` prove restore is limited to Markdown snapshots inside the active document's configured store with matching metadata | Browser/native restore workflow proof and broader storage-mode QA. |
