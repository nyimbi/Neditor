...
| `pnpm run test:tauri-webdriver` | Skipped on macOS | This host still records the official unsupported WKWebView-driver skip, but `.tmp/desktop-webdriver/report.json` now includes the supported Windows/Linux workflow plan with real Markdown save/open/rename/duplicate/reveal plus real HTML export write assertions, `.tmp/desktop-webdriver/native-workflow-file.md`, `.tmp/desktop-webdriver/native-workflow-renamed.md`, `.tmp/desktop-webdriver/native-workflow-duplicate.md`, and `.tmp/desktop-webdriver/native-workflow-report.json` smoke paths. |
