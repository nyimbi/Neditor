...
| `openapi`, `json-schema` | API metadata, terms/contact/license details, tags, external documentation, API operations, security requirements, request/response contracts, response headers/links/examples, callbacks, webhooks, discriminators, component schemas, nested fields, composition, object/conditional/content keywords, draft-07 tuple `items`, boolean schemas, definitions, and schema constraints. |
...
| --- | --- |
| `vega-lite` | Inline JSON specs with `data.values`, `encoding.x.field`, `encoding.y.field`, optional `encoding.color.field` grouping/series, optional `encoding.size.field` bubble/symbol sizing, optional `encoding.text.field` labels, `encoding.x.title`/`encoding.y.title`, numeric y values including negatives with a real zero baseline, y aggregates (`sum`, `mean`, `average`, `min`, `max`), and `bar`, `line`, `point`, `circle`, `square`, `area`, `tick`, `text`, or `rule` marks. Rule marks support numeric `x`/`y` field values or numeric `x.datum`/`y.datum` constants for threshold and reference-line previews. |
