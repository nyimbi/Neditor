What remains to achieve the full vision:
  ...
  1. Real semantic exporters
     ...
     AST, DOCX, PDF, PPTX, and bundle conformance fixtures, DOCX native TOC fields now honor front matter TOC depth, and layout orientation metadata maps to
     HTML/PDF/DOCX page geometry. Still needed: richer paged-layout reflow beyond static section controls.
  ...
  7. Backend modularization
     ...
     builder helpers. The transform and external-transform test cluster now lives in a dedicated nested test module instead of the root backend fixture file. Still needed:
     split the remaining large test fixture clusters further as the feature set grows.
