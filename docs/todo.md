What remains to achieve the full vision:
  ...
  2. Canonical document AST
     ...
     can inspect section layout before files are written. Semantic figure/equation inventory now counts AST blocks instead of scanning raw Markdown strings, and layout shortcode tokens
     now pass through interpolation without false missing-variable warnings, so semantic layout directives do not make readiness reports noisy.
