What remains to achieve the full vision:
  ...
  1. Real semantic exporters
     ...
     through real column slots instead of only recording column metadata. PDF fixture coverage now proves large tables continue into the next section column. Still needed:
     richer paged-layout behavior for floats, widows/orphans, and more complex table splitting across mixed columns/pages.
  2. Canonical document AST
     ...
     can inspect section layout before files are written. Layout shortcode tokens now pass through interpolation without false missing-variable warnings, so semantic layout directives do not make readiness reports noisy.
  ...
  7. Backend modularization
     ...
     builder helpers. The transform/external-transform, semantic export renderer, export command/readiness, AI cleanup, table/formula export, review/provenance, and media package
     test clusters now live in dedicated nested test modules instead of the root backend fixture file. Still needed: split the remaining large compiler and Git fixture clusters further as the feature set grows.
