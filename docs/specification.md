...
Markdown reader setup guidance and machine-readable status reporting, setup
diagnostics, transform handler discovery, release-readiness report inspection,
release evidence report status inspection, and a redaction-safe support bundle
that combines setup diagnostics with release evidence, spec-completion summaries,
and transform-engine probe summaries plus standard release evidence report
statuses. The CLI must support JSON output for automation and strict non-zero
modes for release gates so non-technical support teams can verify an
installation without learning the developer toolchain.
