...
Use **File -> Deploy CLI Globally** in NEditor when you want the packaged `ned`
helper made available from normal terminal windows. The same action is available
from the File toolbar, command palette, and **Settings -> Files -> Command line
and default reader**; it installs a user-level launcher, reports the exact
target path, and shows the PATH command to run if your shell cannot see the
target directory yet. For scripted setup, IT imaging, or help desk verification,
use `ned deploy-cli --status --json` to preview the target and `ned deploy-cli
--json` to install it without opening the app. `ned deploy-cli` refuses to
replace an unrelated existing `ned` command unless `--overwrite` is passed
explicitly. It also refuses to install the tiny development placeholder
generated by the Tauri build script, so release packages must run `pnpm run
prepare:sidecars` before distribution.
...
template discovery, and use
`ned templates --markdown report --title "Weekly Operating Report" --workspace . --fill-profile`
to preview a complete starter document with saved business identity values
merged in before creating a file. `ned new client-proposal.md --template proposal
--workspace . --fill-profile --json` creates the same profile-aware starter on
disk while leaving unknown document-specific placeholders visible for completion.
`ned transform-templates` exposes the same built-in
...
- Data transforms for CSV, TSV, XLSX data sources, JSON, YAML, SQLite SQL,
  ...
  while the References manager exposes a file picker, type inference, a
  binary-safe **Copy to data folder** action for turning picked files into
  portable `data/...` paths, and worksheet fields for non-technical setup.
