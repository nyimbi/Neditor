...
unless `--overwrite` is passed explicitly. It also refuses to install the tiny
development placeholder generated by the Tauri build script, so release
packages must run `pnpm run prepare:sidecars` before distribution.
