...
pub(crate) fn reject_all_suggestions(document: String) -> String {
	...
	result = accept_all_of_kind(result, SUGGEST_DELETE_OPEN, SUGGEST_CLOSE, END_DELETE, false);
	// Reject all insert suggestions (remove text)
	result = accept_all_of_kind(result, SUGGEST_INSERT_OPEN, SUGGEST_CLOSE, END_INSERT, false);
	result
}
...
fn accept_all_of_kind(mut doc: String, open: &str, close: &str, end: &str, accept: bool) -> String {
	loop {
		...
		let Some(close_pos) = doc[start..].find(close) else { break };
		let content_start = start + close_pos + close.len();
		let Some(end_pos) = doc[content_start..].find(end) else { break };
...
pub(crate) fn list_suggestions(document: String) -> Vec<Suggestion> {
	...
	parse_suggestions_of_kind(
		&document, SUGGEST_DELETE_OPEN, SUGGEST_CLOSE, END_DELETE, "delete", &mut suggestions,
	);
	parse_suggestions_of_kind(
		&document, SUGGEST_INSERT_OPEN, SUGGEST_CLOSE, END_INSERT, "insert", &mut suggestions,
	);
...
fn parse_suggestions_of_kind(
	...
	loop {
		...
		out.push(Suggestion { id, kind: kind.to_string(), text, author, line });
