...
fn validate_export_settings(
    ...
    validate_latex_template_option(options, diagnostics);
    validate_transform_export_settings(options, diagnostics);
    validate_target_specific_export_options(target, options, diagnostics);
}

fn validate_latex_template_option(options: &Value, diagnostics: &mut Vec<DocumentDiagnostic>) {
    let Some(latex_template) = options.get("latexTemplate") else {
        return;
    };
    let Some(template_id) = latex_template.as_str() else {
        diagnostics.push(diag(
            "error",
            "latexTemplate must be a string.",
            None,
            None,
            Some("Choose a built-in or saved custom LaTeX template profile id."),
        ));
        return;
    };
    let builtin = matches!(
    ...
    if validate_custom_latex_template_profile(options, template_id, diagnostics) {
        return;
    }
    diagnostics.push(diag(
        "error",
        "latexTemplate must be article, business-report, proposal, rfp-response, technical-report, academic-paper, textbook, book, or a saved custom profile with latexTemplateProfile.",
        None,
        None,
        Some("Choose a built-in profile or save the custom LaTeX template before exporting."),
    ));
}

fn validate_custom_latex_template_profile(
    options: &Value,
    template_id: &str,
    diagnostics: &mut Vec<DocumentDiagnostic>,
) -> bool {
    let Some(profile) = options.get("latexTemplateProfile") else {
        return false;
    };
    let Some(fields) = profile.as_object() else {
        diagnostics.push(diag(
            "error",
            "latexTemplateProfile must be an object.",
            None,
            None,
            Some("Use the saved custom LaTeX template profile generated by the Export panel."),
        ));
        return false;
    };
    if fields.get("id").and_then(Value::as_str) != Some(template_id) {
        diagnostics.push(diag(
            "error",
            "latexTemplateProfile.id must match latexTemplate.",
            None,
            None,
            Some("Reselect or resave the custom LaTeX template profile."),
        ));
        return false;
    }
    for key in [
        ...
        if fields.get(key).is_some_and(|value| !value.is_string()) {
            diagnostics.push(diag(
                "error",
                format!("latexTemplateProfile.{key} must be a string."),
                None,
                None,
                Some("Use the custom LaTeX template editor to normalize this profile."),
            ));
            return false;
        }
    }
        ...
        .is_some_and(|value| !value.is_boolean())
    {
        diagnostics.push(diag(
            "error",
            "latexTemplateProfile.chapterStyle must be true or false.",
            None,
            None,
            Some("Use a boolean value for custom template heading style."),
        ));
        return false;
    }
    let Some(document_class) = fields
        .get("documentClass")
        ...
        diagnostics.push(diag(
            "error",
            "latexTemplateProfile.documentClass is required.",
            None,
            None,
            Some("Set a LaTeX document class such as article, report, book, memoir, IEEEtran, or elsarticle."),
        ));
        return false;
    };
    if !document_class
        ...
        .all(|byte| byte.is_ascii_alphanumeric() || byte == b'-' || byte == b'_')
    {
        diagnostics.push(diag(
            "error",
            "latexTemplateProfile.documentClass contains unsupported characters.",
            None,
            None,
            Some("Use the class name only; put packages and preamble commands in the package/header fields."),
        ));
        return false;
    }
    match fields.get("packages") {
        Some(Value::Array(packages)) => {
            if packages.len() > 24
                || packages
                    .iter()
                    .any(|item| item.as_str().is_none_or(|line| line.trim().len() > 180))
            {
                diagnostics.push(diag(
                    "error",
                    "latexTemplateProfile.packages must contain up to 24 short package lines.",
                    None,
                    None,
                    Some(
                        "Keep only required package or preamble lines in the saved custom profile.",
                    ),
                ));
                return false;
            }
        ...
        Some(_) => {
            diagnostics.push(diag(
                "error",
                "latexTemplateProfile.packages must be a list of strings.",
                None,
                None,
                Some("Use one LaTeX package or preamble line per row in the template editor."),
            ));
            return false;
        }
        None => {}
