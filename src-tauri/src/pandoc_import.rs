...
pub(crate) fn import_document(request: ImportRequest) -> Result<ImportResponse, String> {
    ...
    let format = match ext.as_str() {
        ...
        other => {
            return Err(format!(
                "Unsupported import format: .{other}. Supported: docx, pptx, odt, rtf, html."
            ))
        }
    };
    // G21: resolve pandoc via which to prevent PATH substitution attacks.
    let pandoc_bin = which::which("pandoc").map_err(|_| {
        "pandoc is not installed. Install it from pandoc.org to enable document import.".to_string()
    })?;
    ...
    let output = Command::new(pandoc_bin)
        .args([
            ...
            format,
            "-t",
            "markdown-raw_html+pipe_tables+yaml_metadata_block",
            "--wrap=none",
            "--", // G21: prevent path starting with "-" being interpreted as a flag
