...
fn write_rendered_export_audit_artifacts(artifacts: RenderedExportAuditArtifacts<'_>) {
    ...
    fs::write(
        root.join("README.md"),
        "# Rendered Export Audit\n\nThis directory is generated by `pnpm run test:rendered-exports`.\n\nOpen `manual-review.html` first, then open the HTML/PDF/DOCX/PPTX files in native viewers for manual visual review, inspect the LaTeX source, and inspect the ZIP packages for manifest-backed publishing and Google Docs handoff evidence. The JSON report records target paths, sizes, hashes, and the manual checklist; `viewer-proof.json` records executable package/viewer assertions.\n",
    )
