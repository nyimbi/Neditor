...
fn find_in_dir(root: &PathBuf, dir: &PathBuf, target: &str, results: &mut Vec<Backlink>) {
    ...
    for entry in entries.flatten() {
        ...
        if path.is_dir() {
            find_in_dir(root, &path, target, results);
            continue;
        }
        ...
        for (li, line) in content.lines().enumerate() {
            ...
            if lower.contains(&format!("[[{target}]]"))
                || lower.contains(&format!("include: {target}"))
            {
...
#[tauri::command]
pub(crate) fn find_unlinked_mentions(
    title: String,
    workspace_root: String,
) -> Result<Vec<Backlink>, String> {
    let root = PathBuf::from(&workspace_root);
...
fn find_unlinked_in_dir(root: &PathBuf, dir: &PathBuf, title: &str, results: &mut Vec<Backlink>) {
    ...
    for entry in entries.flatten() {
        ...
        if path.is_dir() {
            find_unlinked_in_dir(root, &path, title, results);
            continue;
        }
        if path.extension().and_then(|e| e.to_str()) != Some("md") {
            continue;
        ...
        for (li, line) in content.lines().enumerate() {
            ...
            if lower.contains(&format!("[[{title}]]")) {
                continue;
            }
            ...
            if let Some(pos) = lower.find(title) {
                ...
                let after_ok =
                    after_pos >= lower.len() || !bytes[after_pos].is_ascii_alphanumeric();
