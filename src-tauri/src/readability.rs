...
mod tests {
	...
	fn test_syllable_counts() {
		...
		assert_eq!(count_syllables("because"), 3);
	...
	fn test_simple_text() {
		...
		assert!(stats.flesch_reading_ease > 70.0, "simple text should score high ease");
		assert!(stats.flesch_kincaid_grade < 5.0, "simple text should be low grade");
	}
