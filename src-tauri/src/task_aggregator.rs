...
fn extract_tags(text: &str) -> Vec<String> {
	...
	while let Some(ch) = chars.next() {
		if ch == '#' {
			let tag: String = chars.by_ref().take_while(|c| c.is_alphanumeric() || *c == '-' || *c == '_').collect();
			if !tag.is_empty() {
				tags.push(tag);
			}
		}
...
#[tauri::command]
pub(crate) fn collect_workspace_tasks(workspace_root: String) -> Result<Vec<WorkspaceTask>, String> {
