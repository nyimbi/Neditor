...
pub(crate) fn compare_documents(request: CompareRequest) -> Result<CompareResponse, String> {
    let text_a =
        fs::read_to_string(&request.path_a).map_err(|e| format!("Cannot read file A: {e}"))?;
    let text_b =
        fs::read_to_string(&request.path_b).map_err(|e| format!("Cannot read file B: {e}"))?;
    let label_a = request
        .label_a
        .unwrap_or_else(|| request.path_a.split('/').last().unwrap_or("A").to_string());
    let label_b = request
        .label_b
        .unwrap_or_else(|| request.path_b.split('/').last().unwrap_or("B").to_string());
