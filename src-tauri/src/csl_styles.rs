...
#[tauri::command]
pub(crate) fn list_installed_csl_styles(
    csl_dir: Option<String>,
) -> Result<Vec<CslStyleInfo>, String> {
    let search_dirs: Vec<PathBuf> = if let Some(dir) = csl_dir {
        ...
        if dirs.is_empty() {
            eprintln!(
                "[csl_styles] HOME and XDG_DATA_HOME unset — no default CSL directories to scan"
            );
        }
        dirs
    };
    ...
    for dir in &search_dirs {
        ...
        for entry in entries.flatten() {
            let path = entry.path();
            if path.extension().and_then(|e| e.to_str()) != Some("csl") {
                continue;
            }
            let filename = path
                .file_name()
