...
fn render_transform(
    ...
    let html = match name {
        ...
        "qr" => render_qr_svg(body, &mut artifact_diags, diagnostics),
...
fn render_qr_matrix(payload: &[u8]) -> Result<Vec<Vec<bool>>, String> {
...
"#
    ...
    fn qr_matrix_reserves_finder_separators() {
        let matrix = render_qr_matrix(b"HELLO").expect("qr matrix");
        ...
        assert!(render_qr_matrix(&vec![b'x'; 79]).is_err());
    }
