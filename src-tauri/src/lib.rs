...
fn render_full_html(response: &CompileResponse, options: &Value) -> String {
...
fn render_docx_bytes(response: &CompileResponse, options_value: &Value) -> Result<Vec<u8>, String> {
...
fn render_markdown_bundle_bytes(
...
"#
    ...
    fn compiler_builds_document_ast_blocks_for_exports() {
        ...
        let exported = export_text(&response, &json!({}));
