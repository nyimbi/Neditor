...
test("support bundle handoff summarizes setup release and spec assignments", () => {
  const report = {
    ...
    specActionPlan: {
      ...
      workOrders: [
        ...
        {
          id: "001-manual-review-layout",
          owner: "Named manual reviewer",
          specSection: "9.15 Advanced layout",
          requirementArea: "Columns, breaks, headers, and footers",
          classification: "manual-review",
          remainingGap: "Attach native-viewer layout review proof.",
          runbooks: ["runbooks/manual-review.md"],
          returns: [".tmp/manual-review/001-manual-review-layout/signoff.json"],
          validatorCommands: ["pnpm run test:rendered-exports", "pnpm run check:manual-review"],
          matrixClosureCommand: "pnpm run check:spec-completion",
          readyToSend: true,
        },
      ],
  ...
  const manualKit = supportBundleManualReviewKitMarkdown(report, "2026-05-30T00:00:00.000Z");
  ok(manualKit.includes("## Manual Review Sign-Off Kit"));
  ok(manualKit.includes("Manual review work orders: 1"));
  ok(manualKit.includes("001-manual-review-layout"));
  ok(manualKit.includes("neditor.manual-review.signoff.v1"));
...
test("Ollama provider profiles support direct AI workflows and deep research sizing", async () => {
  ...
  equal((standaloneWithEmptyBibliography.match(/```bibliography/g) || []).length, 2);
  ok(standaloneWithEmptyBibliography.includes('"id": "agency2026"'));

  const standaloneWithExistingSourceBibliography = deepResearchDocumentMarkdown(
    settings,
    '# Draft\n\n```bibliography\n[{ "id": "agency2026", "type": "webpage", "title": "Policy Evidence", "URL": "https://agency.gov/policy.pdf" }]\n```\n\nBody with source review.',
    [
      {
        index: 1,
        query: "AI procurement controls policy",
...
test("configuration setup helpers score readiness and generate context-aware assistance", () => {
  ...
  const releaseAssistance = buildConfigurationSetupStepAssistance({
    step: configurationSetupStepById("release"),
    status,
    setupSummary: formatConfigurationSetupSummary(status),
    setupNotesWordCount: 12,
    businessDone: 7,
...
test("workbench command bar exposes icon display controls and workflow groups", () => {
  const app = readFileSync("src/App.vue", "utf8")
    ...
    + readFileSync("src/panels/sidebar/FilesPanel.vue", "utf8")
    + readFileSync("src/panels/sidebar/OutlinePanel.vue", "utf8")
    + readFileSync("src/panels/sidebar/LayoutPanel.vue", "utf8")
    + readFileSync("src/panels/sidebar/TablesPanel.vue", "utf8")
    + readFileSync("src/panels/sidebar/TemplatesPanel.vue", "utf8")
    + readFileSync("src/panels/sidebar/ReferencesPanel.vue", "utf8")
    + readFileSync("src/panels/sidebar/ExportsPanel.vue", "utf8")
    ...
    + readFileSync("src/panels/AiCleanupModal.vue", "utf8");
  ...
  ok(app.includes("floating-toolbar-restore"));
  ok(app.includes("workspace-toolbar-restore"));
  ok(app.includes("toolbar-visibility-menu"));
  ok(app.includes("toolbarVisibilityMenuOpen"));
  ok(app.includes("Toolbar visibility menu"));
  ok(app.includes("Toolbars hidden"));
  ok(app.includes("Toolbars hidden:"));
  ok(app.includes("Show all toolbars"));
  ok(app.includes("Open View > Toolbars"));
  ok(app.includes("hiddenToolbarNames"));
  ok(app.includes("hiddenToolbarSummary"));
  ok(app.includes("hiddenToolbarTrayPrompt"));
  ok(app.includes("hiddenToolbarRecoveryPrompt"));
  ok(app.includes("hiddenToolbarWorkspacePrompt"));
  ok(app.includes("hiddenToolbarWorkspaceHelpText"));
  ok(app.includes("hiddenToolbarTrayHelpText"));
  ok(app.includes("hiddenToolbarFloatingHint"));
  ok(app.includes("hiddenToolbarVisibilityMenuHelpText"));
  ok(app.includes("hiddenToolbarFloatingHelpText"));
  ok(app.includes("Use Show all or any row button here to unhide toolbars; View > Toolbars has the same controls."));
  ok(app.includes("Show all hidden toolbar rows"));
  ok(app.includes("Click to show the ${first.label} toolbar"));
  ok(app.includes("Click to unhide ${first.label}"));
  ok(app.includes("Show all"));
  ok(app.includes("Show {{ row.label }}"));
  ok(app.includes("Show ${row.label} toolbar"));
  ok(app.includes("Show ${row.label} Toolbar"));
  ok(app.includes("use the Toolbars hidden menu, floating restore button, hidden toolbars tray, or View > Toolbars to show it again"));
  ok(app.includes("toolbarRowLabel"));
  ok(app.includes("showToolbarRow"));
  ok(app.includes("showAllHiddenToolbars"));
  ok(app.includes("showHiddenToolbarFromMenu"));
  ok(app.includes("unhide"));
  ...
  ok(app.includes("Hide side panels, status, preview, and toolbar rows while keeping visible restore controls."));
  ...
  ok(app.includes("deep research, outline, composition"));
  ok(app.includes("claim evidence, publishing preflight, export, and setup"));
  ok(app.includes("Use this file as the canonical demo, screenshot, QA, and release evidence source."));
  ok(app.includes('id: "deep-research"'));
  ok(app.includes('id: "claim-evidence"'));
  ok(app.includes('id: "publishing-preflight"'));
  ok(app.includes('id: "setup-delivery"'));
  ok(app.includes("Local source library search, bibliography population, source conflict review, and audit packets"));
  ok(app.includes("Claim-source evidence matrix, citation TODOs, and approval metadata blockers"));
  ok(app.includes("Configuration Center, Deploy CLI, default Markdown reader guidance, and support bundle"));
  ok(app.includes("Confirm local source-library and claim-source matrix suggestions are reviewed before citations are accepted."));
  ok(app.includes("Confirm publishing preflight stays in dry-run mode until endpoint, metadata, and authorization are approved."));
  ...
  ok(app.includes("specActionPlan"));
  ok(app.includes("releaseCandidate"));
  ok(app.includes("Action plan"));
  ok(app.includes("Release evidence action plan"));
  ok(app.includes("Specification work orders"));
  ok(app.includes("Release candidate"));
  ...
  ok(cli.includes("--spec-work-orders"));
  ok(cli.includes("--release-candidate-dir"));
  ok(cli.includes("release-candidate"));
  ok(cli.includes("sourceCurrent"));
  ok(cli.includes("neditor.ned-release-action-plan.v1"));
  ok(cli.includes("neditor.ned-spec-action-plan.v1"));
  ok(cli.includes("neditor.ned-release-candidate.v1"));
  ok(cli.includes("neditor.ned-open.v1"));
  ok(cli.includes("neditor.ned-new.v1"));
  ok(cli.includes("templateDetails"));
  ok(cli.includes('"sow"'));
  ok(cli.includes('"capability-statement"'));
  ok(cli.includes('"case-study"'));
  ...
  ok(cli.includes("neditor.ned-template.v1"));
...
test("local verification scripts expose local baseline checks", () => {
  ...
  ok(tableEditorSignoffCollector.includes("Table editor manual signoff must be collected from a clean Git tree"));
  ok(tableEditorSignoffCollector.includes("Validate it with: NEDITOR_TABLE_EDITOR_SIGNOFF"));
  ok(tableEditorSignoffCollector.includes("supportedHostResults"));
  ok(tableEditorSignoffCollector.includes("unresolvedBlockers: []"));
  ...
  ok(evidenceKitChecker.includes("const expectedMinimumTemplateCount = 19"));
  ...
  ok(evidenceKitChecker.includes("copiedTemplates must include at least ${expectedMinimumTemplateCount} entries"));
  ...
  ok(releaseCandidate.includes("Validate it before native/rendered probes that can exhaust browser launch slots on macOS."));
  ok(releaseCandidate.includes("currentBrowserWorkflowEvidence"));
  ok(releaseCandidate.includes("reuse .tmp/e2e-browser/report.json"));
  ok(releaseCandidate.includes("Run `pnpm run test:e2e` as a top-level shell command"));
